...
const renderPromoTable=rows=>{const list=Array.isArray(rows)?rows:[];const total=list.length;const activeCount=list.filter(r=>String(r.active)==='true'||r.active===true).length;const totalUses=list.reduce((sum,r)=>sum+(parseInt(r.current_uses||r.currentUses||r.uses_count||r.uses||0,10)||0),0);$('#imsda-promo-summary').text(`Total codes: ${total} | Active: ${activeCount} | Total uses: ${totalUses}`);if(!list.length){$('#imsda-promos-wrap').html('<p>No promo codes yet. Create one using the button above.</p>');return;}const today=new Date();today.setHours(0,0,0,0);const rowsHtml=list.map(r=>{const code=pEsc(r.code||'');const desc=r.description?pEsc(r.description):'<span style="color:#646970;">—</span>';const type=String(r.discount_type||r.type||'percent').toLowerCase();const amount=parseFloat(r.discount_amount||r.amount||0);const discount=type==='fixed'?`$${isNaN(amount)?'0.00':amount.toFixed(2)}`:`${isNaN(amount)?0:amount}%`;const min=parseFloat(r.min_purchase||r.minPurchase||0);const minCell=min>0?`$${min.toFixed(2)}`:'—';const current=parseInt(r.current_uses||r.currentUses||r.uses_count||r.uses||0,10)||0;const max=parseInt(r.max_uses||r.maxUses||0,10)||0;const maxed=max>0&&current>=max;const uses=`<span style="${maxed?'color:#d63638;font-weight:600;':''}">${current} / ${max>0?max:'∞'}</span>`;const expRaw=String(r.expiry_date||r.expiryDate||'').trim();let expiry='<span style="color:#646970;">No expiry</span>';if(expRaw){const expDate=new Date(expRaw+'T00:00:00');const expired=!isNaN(expDate.getTime())&&expDate<today;expiry=`<span style="${expired?'color:#d63638;font-weight:600;':''}">${pEsc(expRaw)}</span>`;}const isActive=String(r.active)==='true'||r.active===true;const active=isActive?'<span style="color:#008a20;">✅ Active</span>':'<span style="color:#646970;">❌ Inactive</span>';return `<tr data-code="${code}"><td><code style="font-size:1em;">${code}</code></td><td>${desc}</td><td>${discount}</td><td>${minCell}</td><td>${uses}</td><td>${expiry}</td><td>${active}</td><td><button class="button button-small imsda-promo-delete" data-code="${code}">Delete</button><span class="imsda-promo-inline-err" style="color:#d63638;margin-left:6px;"></span></td></tr>`;}).join('');$('#imsda-promos-wrap').html(`<table class="widefat striped"><thead><tr><th>Code</th><th>Description</th><th>Discount</th><th>Min Purchase</th><th>Uses</th><th>Expiry</th><th>Active</th><th>Actions</th></tr></thead><tbody>${rowsHtml}</tbody></table>`);};
const loadPromoCodes=()=>{$('#imsda-promos-wrap').html('<p>Loading promo codes…</p>');$.post(ajaxurl,{action:'imsda_reg_admin_action',nonce:nonce,imsda_action:'getPromoCodes',event_slug:slug}).done(function(resp){if(!(resp&&resp.success)){$('#imsda-promos-wrap').html(`<p style="color:#d63638;">${pEsc(resp?.data?.message||'Failed to load promo codes.')}</p>`);$('#imsda-promo-summary').text('');return;}const data=Array.isArray(resp.data?.rows)?resp.data.rows:(Array.isArray(resp.data)?resp.data:[]);renderPromoTable(data);}).fail(function(){$('#imsda-promos-wrap').html('<p style="color:#d63638;">Failed to load promo codes.</p>');$('#imsda-promo-summary').text('');});};
