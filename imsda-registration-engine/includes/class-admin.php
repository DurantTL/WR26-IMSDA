...
app.html(`<p><input id='s'> <button class='button r'>Refresh</button></p><table class='widefat'><thead><tr><th>ID</th><th>Name</th><th>Church</th><th>Payment</th><th>Status</th><th>Checked In</th><th>Actions</th></tr></thead><tbody id='tb'></tbody></table><div id="imsda-edit-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:100000;align-items:center;justify-content:center"><div style="background:#fff;border-radius:6px;padding:24px;max-width:640px;width:94vw;max-height:90vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.25)"><h2 id="imsda-edit-title" style="margin-top:0">Edit Registration</h2><table class="form-table" style="margin:0" id="imsda-edit-body"><!-- rows injected by openEditModal() --></table><p style="margin-top:16px;display:flex;gap:8px"><button class="button button-primary" id="imsda-edit-save">Save Changes</button><button class="button" id="imsda-edit-cancel">Cancel</button><span id="imsda-edit-status" style="color:#d63638;align-self:center;margin-left:4px"></span></p></div></div><div id="imsda-transfer-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:100000;align-items:center;justify-content:center"><div style="background:#fff;border-radius:6px;padding:24px;max-width:620px;width:94vw;max-height:90vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.25)"><h2 style="margin-top:0">Transfer Registration</h2><div style="background:#fff3cd;border-radius:4px;padding:10px 12px;margin-bottom:16px"><strong>Original:</strong> <span id="imsda-tr-orig-name"></span> (<code id="imsda-tr-orig-id"></code>)<br><em style="font-size:.85em;color:#856404">The original registration will be marked transferred and locked. A full audit trail is saved.</em></div><h3 style="margin:0 0 10px">New Registrant Details</h3><table class="form-table" style="margin:0"><tr><th>First Name *</th><td><input type="text" id="imsda-tr-fn" class="regular-text"></td></tr><tr><th>Last Name *</th><td><input type="text" id="imsda-tr-ln" class="regular-text"></td></tr><tr><th>Email *</th><td><input type="email" id="imsda-tr-em" class="regular-text"></td></tr><tr><th>Phone</th><td><input type="tel" id="imsda-tr-ph" class="regular-text"></td></tr><tr><th>Church</th><td><input type="text" id="imsda-tr-ch" class="regular-text"></td></tr></table><h3 style="margin:16px 0 10px">Transfer Details</h3><table class="form-table" style="margin:0"><tr><th>Reason</th><td><textarea id="imsda-tr-reason" class="large-text" rows="2" placeholder="e.g. Registrant can no longer attend"></textarea></td></tr><tr><th>Refund Notes</th><td><textarea id="imsda-tr-refund" class="large-text" rows="2" placeholder="e.g. Full refund $120 via Square"></textarea></td></tr><tr><th>Admin Notes</th><td><textarea id="imsda-tr-notes" class="large-text" rows="2" placeholder="Internal only"></textarea></td></tr></table><p id="imsda-tr-status" style="color:#d63638;margin-top:8px"></p><p style="display:flex;gap:8px"><button class="button button-primary" id="imsda-tr-save">Transfer Registration</button><button class="button" id="imsda-tr-cancel">Cancel</button></p></div></div>`);
let currentRegId = '';
...
function openEditModal(reg) { currentRegId = reg.registrationId || reg.id || ''; $('#imsda-edit-title').text('Edit: ' + (reg.firstName||'') + ' ' + (reg.lastName||'')); var defs = [['firstName','First Name','text'],['lastName','Last Name','text'],['phone','Phone','tel'],['church','Church','text'],['arrivalDate','Arrival Date','date'],['departureDate','Departure Date','date'],['dietaryNeeds','Dietary Needs','textarea'],['emergencyContactName','Emergency Contact Name','text'],['emergencyContactPhone','Emergency Contact Phone','tel'],['specialNeeds','Special Needs','textarea'],['adminNotes','Admin Notes','textarea']]; var h = ''; defs.forEach(function(d) { var val = (reg[d[0]] || '').toString().replace(/"/g,'&quot;'); var inp = d[2]==='textarea' ? '<textarea name="'+d[0]+'" class="large-text" rows="2">'+esc(reg[d[0]]||'')+'</textarea>' : '<input type="'+d[2]+'" name="'+d[0]+'" value="'+val+'" class="regular-text">'; h += '<tr><th style="width:200px">'+d[1]+'</th>' + '<td>'+inp+'</td></tr>'; }); $('#imsda-edit-body').html(h); $('#imsda-edit-status').text(''); $('#imsda-edit-modal').css('display','flex'); }
function openTransferModal(regId, regName) { currentRegId = regId; $('#imsda-tr-orig-name').text(regName); $('#imsda-tr-orig-id').text(regId); $('#imsda-tr-fn,#imsda-tr-ln,#imsda-tr-em,#imsda-tr-ph,#imsda-tr-ch,#imsda-tr-reason,#imsda-tr-refund,#imsda-tr-notes').val(''); $('#imsda-tr-status').text(''); $('#imsda-transfer-modal').css('display','flex'); }
$(document).on('click', '.imsda-edit-btn', function() { var raw = $(this).data('reg'); var reg = typeof raw === 'string' ? JSON.parse(raw.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&#39;/g,"'")) : raw; openEditModal(reg); });
$(document).on('click', '.imsda-transfer-btn', function() { openTransferModal($(this).data('id'), $(this).data('name')); });
$('#imsda-edit-save').on('click', function() { var fields = {}; $('#imsda-edit-body input, #imsda-edit-body textarea').each(function() { if ($(this).attr('name')) fields[$(this).attr('name')] = $(this).val(); }); $('#imsda-edit-status').text('Saving…').css('color','#646970'); $.post(imsda_reg.ajax_url, { action: 'imsda_reg_admin_action', imsda_action: 'adminEditRegistration', event_slug: slug, registration_id: currentRegId, fields: fields, nonce: imsda_reg.nonce }, function(r) { if (r && r.success) { $('#imsda-edit-modal').hide(); loadRegistrations(); } else { $('#imsda-edit-status').text('❌ ' + (r&&r.message ? r.message : 'Save failed')).css('color','#d63638'); } }); });
$('#imsda-edit-cancel').on('click', function() { $('#imsda-edit-modal').hide(); });
$('#imsda-edit-modal').on('click', function(e) { if ($(e.target).is('#imsda-edit-modal')) $(this).hide(); });
$('#imsda-tr-save').on('click', function() { var fn = $('#imsda-tr-fn').val().trim(); var ln = $('#imsda-tr-ln').val().trim(); var em = $('#imsda-tr-em').val().trim(); if (!fn || !ln || !em) { $('#imsda-tr-status').text('First name, last name, and email are required.'); return; } if (!/\S+@\S+\.\S+/.test(em)) { $('#imsda-tr-status').text('Invalid email address.'); return; } $('#imsda-tr-status').text('Processing…').css('color','#646970'); $.post(imsda_reg.ajax_url, { action: 'imsda_reg_admin_action', imsda_action: 'transferRegistration', event_slug: slug, registration_id: currentRegId, new_first_name: fn, new_last_name: ln, new_email: em, new_phone: $('#imsda-tr-ph').val(), new_church: $('#imsda-tr-ch').val(), reason: $('#imsda-tr-reason').val(), refund_notes: $('#imsda-tr-refund').val(), admin_notes: $('#imsda-tr-notes').val(), nonce: imsda_reg.nonce }, function(r) { if (r && r.success) { $('#imsda-transfer-modal').hide(); alert('✅ Transfer complete. Confirmation emails sent to both parties.'); loadRegistrations(); } else { $('#imsda-tr-status').text('❌ ' + (r&&r.message ? r.message : 'Transfer failed')).css('color','#d63638'); } }); });
$('#imsda-tr-cancel').on('click', function() { $('#imsda-transfer-modal').hide(); });
$('#imsda-transfer-modal').on('click', function(e) { if ($(e.target).is('#imsda-transfer-modal')) $(this).hide(); });
loadRegistrations();
