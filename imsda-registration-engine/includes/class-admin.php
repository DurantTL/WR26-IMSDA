...
const wlGetRows=r=>Array.isArray(r.waitlist)?r.waitlist:[];
let waitlistRows=[];
...
const loadStats=()=>ajax('getCheckInStats',{},r=>{const stats=(r&&r.stats)||{};const reg=parseInt(stats.total||0,10),inC=parseInt(stats.checkedIn||0,10),ny=parseInt(stats.notCheckedIn||Math.max(0,reg-inC),10),pct=parseInt(stats.percent||0,10),pending=parseInt(stats.paymentsPending||0,10);const pcol=pct>=80?'#00a32a':(pct>=50?'#dba617':'#d63638');$('#imsda-ci-stats').html(`<div style='display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px'><div class='imsda-stat-card'><span class='imsda-stat-label'>Total Registered</span><span class='imsda-stat-value'>${reg}</span></div><div class='imsda-stat-card'><span class='imsda-stat-label'>Checked In</span><span class='imsda-stat-value' style='color:#00a32a'>${inC}</span></div><div class='imsda-stat-card'><span class='imsda-stat-label'>Not Yet</span><span class='imsda-stat-value' style='color:${ny>0?'#d63638':'#646970'}'>${ny}</span></div><div class='imsda-stat-card'><span class='imsda-stat-label'>Check-In %</span><span class='imsda-stat-value' style='color:${pcol}'>${pct}%</span></div><div class='imsda-stat-card'><span class='imsda-stat-label'>Payments Pending</span><span class='imsda-stat-value' style='color:${pending>0?'#dba617':'#00a32a'}'>${pending}</span></div></div>`);const churches=(r.byChurch||[]).sort((a,b)=>String(a.church||'').localeCompare(String(b.church||'')));$('#imsda-ci-church').html(`<table class='widefat striped' style='margin-top:12px'><thead><tr><th>Church</th><th>Registered</th><th>Checked In</th><th>%</th></tr></thead><tbody>${churches.map(c=>{const cp=(c.total>0)?Math.round((c.checkedIn/c.total)*100):0;const cc=cp>=80?'#00a32a':(cp>=50?'#dba617':'#d63638');return `<tr><td>${c.church||''}</td><td>${c.total||0}</td><td>${c.checkedIn||0}</td><td style='color:${cc};font-weight:600'>${cp}%</td></tr>`;}).join('')}</tbody></table>`);$('#imsda-ci-last').text('Last updated: '+(new Date()).toLocaleTimeString());});
const loadRecent=()=>ajax('getRecentCheckIns',{},r=>{const rows=(r.registrations||[]);$('#imsda-ci-recent').html(`<table class='widefat striped'><thead><tr><th>Time</th><th>Name</th><th>Church</th><th>Method</th><th>Admin/Device</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${x.timestamp||''}</td><td>${x.name||''}</td><td>${x.church||''}</td><td>${x.method||''}</td><td>${x.adminUser||''}</td></tr>`).join('')}</tbody></table>`);});
...
const renderPromoTable=rows=>{const list=Array.isArray(rows)?rows:[];const total=list.length;const activeCount=list.filter(r=>String(r.active)==='true'||r.active===true).length;const totalUses=list.reduce((sum,r)=>sum+(parseInt(r.currentUses||0,10)||0),0);$('#imsda-promo-summary').text(`Total codes: ${total} | Active: ${activeCount} | Total uses: ${totalUses}`);if(!list.length){$('#imsda-promos-wrap').html('<p>No promo codes yet. Create one using the button above.</p>');return;}const today=new Date();today.setHours(0,0,0,0);const rowsHtml=list.map(r=>{const code=pEsc(r.code||'');const desc=r.description?pEsc(r.description):'<span style="color:#646970;">—</span>';const type=String(r.discountType||'percent').toLowerCase();const amount=parseFloat(r.discountAmount||0);const discount=type==='fixed'?`$${isNaN(amount)?'0.00':amount.toFixed(2)}`:`${isNaN(amount)?0:amount}%`;const min=parseFloat(r.minPurchase||0);const minCell=min>0?`$${min.toFixed(2)}`:'—';const current=parseInt(r.currentUses||0,10)||0;const max=parseInt(r.maxUses||0,10)||0;const maxed=max>0&&current>=max;const uses=`<span style="${maxed?'color:#d63638;font-weight:600;':''}">${current} / ${max>0?max:'∞'}</span>`;const expRaw=String(r.expiryDate||'').trim();let expiry='<span style="color:#646970;">No expiry</span>';if(expRaw){const expDate=new Date(expRaw+'T00:00:00');const expired=!isNaN(expDate.getTime())&&expDate<today;expiry=`<span style="${expired?'color:#d63638;font-weight:600;':''}">${pEsc(expRaw)}</span>`;}const isActive=String(r.active)==='true'||r.active===true;const active=isActive?'<span style="color:#008a20;">✅ Active</span>':'<span style="color:#646970;">❌ Inactive</span>';return `<tr data-code="${code}"><td><code style="font-size:1em;">${code}</code></td><td>${desc}</td><td>${discount}</td><td>${minCell}</td><td>${uses}</td><td>${expiry}</td><td>${active}</td><td><button class="button button-small imsda-promo-delete" data-code="${code}">Delete</button><span class="imsda-promo-inline-err" style="color:#d63638;margin-left:6px;"></span></td></tr>`;}).join('');$('#imsda-promos-wrap').html(`<table class="widefat striped"><thead><tr><th>Code</th><th>Description</th><th>Discount</th><th>Min Purchase</th><th>Uses</th><th>Expiry</th><th>Active</th><th>Actions</th></tr></thead><tbody>${rowsHtml}</tbody></table>`);};
const loadPromoCodes=()=>{$('#imsda-promos-wrap').html('<p>Loading promo codes…</p>');$.post(ajaxurl,{action:'imsda_reg_admin_action',nonce:nonce,imsda_action:'getPromoCodes',event_slug:slug}).done(function(resp){if(!(resp&&resp.success)){$('#imsda-promos-wrap').html(`<p style="color:#d63638;">${pEsc(resp?.data?.message||'Failed to load promo codes.')}</p>`);$('#imsda-promo-summary').text('');return;}const data=resp.promoCodes||[];renderPromoTable(data);}).fail(function(){$('#imsda-promos-wrap').html('<p style="color:#d63638;">Failed to load promo codes.</p>');$('#imsda-promo-summary').text('');});};
