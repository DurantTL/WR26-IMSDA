...
add_shortcode('wr_edit_registration', function(){ wp_enqueue_script('jquery'); $token=sanitize_text_field($_GET['token']??''); if(!$token) return '<p>No registration token found. Please use the link from your confirmation email.</p>'; ob_start(); ?>
<div id="wr26-edit-wrap"><p id="wr26-edit-loading">Loading your registration…</p><form id="wr26-edit-form" style="display:none"><p><strong>Name:</strong> <span id="wr26-edit-name"></span></p><p><em>Email, payment status, and housing cannot be changed here. Contact us if you need to update those fields.</em></p><table class="form-table" style="max-width:600px"><tr><th>First Name</th><td><input type="text" name="first_name" class="regular-text"></td></tr><tr><th>Last Name</th><td><input type="text" name="last_name" class="regular-text"></td></tr><tr><th>Phone</th><td><input type="tel" name="phone" class="regular-text"></td></tr><tr><th>Church</th><td><input type="text" name="church" class="regular-text"></td></tr><tr><th>Dietary Needs</th><td><textarea name="dietary_needs" class="large-text" rows="2"></textarea></td></tr><tr><th>Emergency Contact Name</th><td><input type="text" name="emergency_contact_name" class="regular-text"></td></tr><tr><th>Emergency Contact Phone</th><td><input type="tel" name="emergency_contact_phone" class="regular-text"></td></tr><tr><th>Special Needs</th><td><textarea name="special_needs" class="large-text" rows="2"></textarea></td></tr></table><p><button type="button" id="wr26-edit-save">Save Changes</button> <span id="wr26-edit-status"></span></p></form><div id="wr26-edit-success" style="display:none">✅ Your registration has been updated successfully.</div></div>
<script>window.wr26_edit={ajax_url:+<?php echo wp_json_encode(admin_url('admin-ajax.php')); ?>+,token:+<?php echo wp_json_encode($token); ?>+};jQuery(function($){$.post(wr26_edit.ajax_url,{action:'wr26_get_reg_by_token',token:wr26_edit.token},function(r){if(r&&r.success&&r.registration){var reg=r.registration;$('#wr26-edit-loading').hide();$('#wr26-edit-form').show();$('#wr26-edit-name').text((reg.firstName||'')+' '+(reg.lastName||''));$('input[name=first_name]').val(reg.firstName||'');$('input[name=last_name]').val(reg.lastName||'');$('input[name=phone]').val(reg.phone||'');$('input[name=church]').val(reg.church||'');$('textarea[name=dietary_needs]').val(reg.dietaryNeeds||'');$('input[name=emergency_contact_name]').val(reg.emergencyContactName||'');$('input[name=emergency_contact_phone]').val(reg.emergencyContactPhone||'');$('textarea[name=special_needs]').val(reg.specialNeeds||'');}else{$('#wr26-edit-loading').text('Unable to load registration.');}});$('#wr26-edit-save').on('click',function(){var data={action:'wr26_save_edit',edit_token:wr26_edit.token};$('#wr26-edit-form').find('input,textarea').each(function(){data[$(this).attr('name')]=$(this).val();});$('#wr26-edit-status').text('Saving…');$.post(wr26_edit.ajax_url,data,function(r){if(r&&r.success){$('#wr26-edit-form').hide();$('#wr26-edit-success').show();}else{$('#wr26-edit-status').text('❌ '+(r&&r.message?r.message:'Save failed'));}});});});</script><?php return ob_get_clean(); });
