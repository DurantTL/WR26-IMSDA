...
function recordPayment(payload){try{var registrationId=String((payload&&payload.registrationId)||'');if(!registrationId)return {success:false,message:'Registration not found'};var existing=getRegistrationById(registrationId);if(!existing)return {success:false,message:'Registration not found'};var method=String(payload.paymentMethod||'').toLowerCase();var allowed={cash:true,check:true,square_onsite:true,other:true};if(!allowed[method])method='other';var enteredAmount=Number(payload.amountPaid||0);var baseAmount=enteredAmount>0?enteredAmount:Number(existing.finalAmount||0);var feeInfo=(method==='square_onsite')?calculateSquareFee(baseAmount):{base:baseAmount,fee:0,total:baseAmount};var recordedAmount=feeInfo.total;var notes=[];notes.push('['+new Date().toISOString()+'] On-site payment recorded: '+method);notes.push('Amount: $'+recordedAmount+'.');if(feeInfo.fee>0)notes.push('Includes Square fee. Base: $'+feeInfo.base+', Fee: $'+feeInfo.fee+', Total: $'+feeInfo.total+'.');if(payload.checkNumber)notes.push('Check #: '+payload.checkNumber+'.');if(payload.paymentNotes)notes.push('Notes: '+payload.paymentNotes+'.');notes.push('By: '+(payload.adminUser||'admin')+'.');var updateFields={paymentStatus:'paid_onsite',paymentMethod:method,adminNotes:(existing.adminNotes?existing.adminNotes+'\n':'')+notes.join(' '),finalAmount:recordedAmount};var updated=updateRegistration(registrationId,updateFields);if(!updated.success)return updated;return {success:true,registration:getRegistrationById(registrationId),fee:feeInfo.fee,baseAmount:feeInfo.base,totalCollected:feeInfo.total};}catch(e){return {success:false,message:e.message};}}
