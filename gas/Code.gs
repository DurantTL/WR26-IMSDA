...
function handleRegister(payload){try{
  ...
  var locked=withScriptLock_(function(){
    ...
    if(cap.full||attendeeCount>cap.available)return {success:false,message:cap.full?'Capacity reached':('Only '+cap.available+' seat(s) remain; this registration needs '+attendeeCount+'.'),capacityFull:true,available:cap.available,requested:attendeeCount};
    ...
    if(isPaid){
      ...
      // promo = couponUsed || payload.promo_code, so this only flags underpayment with
